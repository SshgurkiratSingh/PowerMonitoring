...
export default function RedundancyVisualization({ inView }: RedundancyProps) {
  ...
  const [failureMode, setFailureMode] = useState<"none" | "gprs" | "lora">(
    "none"
  );
  ...
  useEffect(() => {
      ...
      const connections = [
        {
          from: nodes.device,
          to: nodes.gprs,
          active: nodes.gprs.active,
          color: "#10b981",
        },
        {
          from: nodes.device,
          to: nodes.lora,
          active: nodes.lora.active,
          color: "#8b5cf6",
        },
        {
          from: nodes.gprs,
    ...
    const animate = () => {
      ...
      packets.forEach((packet, idx) => {
        ...
        packet.draw(ctx);
        if (
          packet.currentWaypoint >= packet.waypoints.length - 1 &&
          packet.progress >= 0.9
        ) {
      ...
      >
        ...
        <div className="mt-6 text-center text-gray-300 relative z-10">
          <p className="text-lg font-medium">
            Dual-Path Communication with Automatic Failover
          </p>
        </div>
      </motion.div>
